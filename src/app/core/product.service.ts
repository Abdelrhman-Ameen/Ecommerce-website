...
export class ProductService {
  ...
  list(query: ProductQuery = {}): Observable<ApiResponse<{ products: Product[]; categories: string[] }>> {
    let params = new HttpParams();
    Object.entries(query).forEach(([key, value]) => {
      if (value !== undefined && value !== '' && value !== false) params = params.set(key, String(value));
    });
    return this.http.get<ApiResponse<{ products: Product[]; categories: string[] }>>(this.api, { params });
  }
