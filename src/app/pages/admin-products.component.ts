...
@Component({
  ...
  template: `
    ...
    @if (formOpen()) { <div class="drawer-backdrop" (click)="closeForm()"></div><aside class="admin-drawer product-editor-drawer"><div class="drawer-head"><div><div class="eyebrow">{{ 'Catalog editor' | translate }}</div><h2>{{ (editing() ? 'Edit product' : 'New product') | translate }}</h2></div><button type="button" (click)="closeForm()"><i class="bi bi-x-lg"></i></button></div><form [formGroup]="form" (ngSubmit)="save()" novalidate>
      ...
      <div class="row g-3"><div class="col-md-6"><div class="form-group-luxe"><div class="field-label-row"><label for="productCategory">{{ 'Main category' | translate }} <span class="required-mark">*</span></label><button type="button" (click)="openCategoryManager('root')"><i class="bi bi-plus-circle me-1"></i>{{ 'Add main category' | translate }}</button></div><select id="productCategory" class="form-select" formControlName="category" [class.is-invalid]="invalid('category')" (change)="categoryChanged()"><option value="">{{ 'Choose category' | translate }}</option>@for (category of rootCategories(); track category.name) { <option [value]="category.name">{{ category.name | titlecase }}</option> }</select><small class="form-hint">{{ 'Categories are managed centrally to prevent duplicates and typos.' | translate }}</small></div></div><div class="col-md-6"><div class="form-group-luxe"><div class="field-label-row"><label for="productSubcategory">{{ 'Subcategory' | translate }} <span class="text-secondary">({{ 'optional' | translate }})</span></label><button type="button" (click)="openCategoryManager('child', form.controls.category.value)" [disabled]="!form.controls.category.value"><i class="bi bi-diagram-3 me-1"></i>{{ 'Add subcategory' | translate }}</button></div><select id="productSubcategory" class="form-select" formControlName="subcategory"><option value="">{{ 'All of main category' | translate }}</option>@for (subcategory of subcategories(form.controls.category.value); track subcategory.name) { <option [value]="subcategory.name">{{ subcategory.name | titlecase }}</option> }</select><small class="form-hint">{{ (form.controls.category.value ? 'Choose a product type or add one here.' : 'Choose a main category first.') | translate }}</small></div></div></div>
    ...
    @if (categoryManagerOpen()) {
      <div class="category-manager-backdrop" (click)="closeCategoryManager()"></div>
      <section class="category-manager-panel" role="dialog" aria-modal="true" [attr.aria-label]="'Manage categories' | translate">
        <div class="drawer-head"><div><div class="eyebrow">{{ 'Catalog structure' | translate }}</div><h2>{{ 'Manage categories' | translate }}</h2></div><button type="button" (click)="closeCategoryManager()" [attr.aria-label]="'Close' | translate"><i class="bi bi-x-lg"></i></button></div>
        <p class="category-manager-intro">{{ 'Create broad shop sections as main categories, then add product types beneath them as subcategories.' | translate }}</p>
        <div class="category-type-switch" role="group" [attr.aria-label]="'What do you want to add?' | translate">
          <button type="button" [class.active]="categoryMode() === 'root'" (click)="chooseCategoryMode('root')"><i class="bi bi-folder-plus"></i><span><strong>{{ 'Main category' | translate }}</strong><small>{{ 'Example: Fashion or Kitchen' | translate }}</small></span></button>
          <button type="button" [class.active]="categoryMode() === 'child'" (click)="chooseCategoryMode('child')"><i class="bi bi-diagram-3"></i><span><strong>{{ 'Subcategory' | translate }}</strong><small>{{ 'Example: Dresses inside Fashion' | translate }}</small></span></button>
        </div>
        <form class="category-create-form" [formGroup]="categoryForm" (ngSubmit)="createCategory()">
          @if (categoryMode() === 'child') { <label for="categoryParent">{{ 'Choose the main category' | translate }} <span class="required-mark">*</span></label><select id="categoryParent" class="form-select mb-3" formControlName="parent"><option value="">{{ 'Choose category' | translate }}</option>@for (root of rootCategories(); track root.name) { <option [value]="root.name">{{ root.name | titlecase }}</option> }</select> }
          <label for="newCategory">{{ (categoryMode() === 'child' ? 'Subcategory name' : 'Main category name') | translate }} <span class="required-mark">*</span></label>
          <div><input id="newCategory" class="form-control" formControlName="name" [placeholder]="(categoryMode() === 'child' ? 'Example: Dresses' : 'Example: Beauty') | translate"><button class="btn btn-primary-luxe" type="submit" [disabled]="categoryForm.invalid || savingCategory() || (categoryMode() === 'child' && !categoryForm.controls.parent.value)">{{ (savingCategory() ? 'Adding...' : (categoryMode() === 'child' ? 'Add subcategory' : 'Add main category')) | translate }}</button></div>
        </form>
        <div class="category-manager-summary"><span><strong>{{ rootCategories().length }}</strong>{{ 'main categories' | translate }}</span><span><strong>{{ categories().length - rootCategories().length }}</strong>{{ 'subcategories' | translate }}</span></div>
        <div class="category-groups">
          @for (category of rootCategories(); track category.name) {
            <section class="category-group">
              <header><div class="category-group-icon"><i class="bi bi-folder2-open"></i></div><div><strong>{{ category.name | titlecase }}</strong><small>{{ category.productCount }} {{ 'products' | translate }} · {{ category.subcategoryCount || 0 }} {{ 'subcategories' | translate }}</small></div><button class="category-add-child" type="button" (click)="prepareSubcategory(category.name)"><i class="bi bi-plus-lg"></i>{{ 'Add subcategory' | translate }}</button>@if (category._id && category.productCount === 0 && !category.subcategoryCount) { <button class="category-delete-button" type="button" (click)="deleteCategory(category)" [attr.aria-label]="'Delete' | translate"><i class="bi bi-trash"></i></button> }</header>
              <div class="category-children">@for (child of subcategories(category.name); track child.name) { <article><span><i class="bi bi-arrow-return-right"></i><strong>{{ child.name | titlecase }}</strong><small>{{ child.productCount }} {{ 'products' | translate }}</small></span>@if (child._id && child.productCount === 0) { <button class="category-delete-button" type="button" (click)="deleteCategory(child)" [attr.aria-label]="'Delete' | translate"><i class="bi bi-trash"></i></button> } @else { <i class="bi bi-lock-fill category-locked" [attr.title]="'Used by products' | translate"></i> }</article> } @empty { <button class="category-empty-child" type="button" (click)="prepareSubcategory(category.name)"><i class="bi bi-plus-circle"></i>{{ 'Add the first subcategory' | translate }}</button> }</div>
            </section>
          } @empty { <div class="empty-state compact">{{ 'Create your first category.' | translate }}</div> }
        </div>
      </section>
    }
  `,
...
export class AdminProductsComponent implements OnInit {
  ...
  readonly rootCategories = computed(() => this.categories().filter((category) => !category.parent).sort((a, b) => a.name.localeCompare(b.name)));
  ...
  readonly categoryMode = signal<CategoryMode>('root');
  readonly form;
  ...
  constructor(private api: ProductService, private admin: AdminService, private imageUpload: ImageUploadService, private toast: ToastService, private route: ActivatedRoute, fb: FormBuilder) {
    ...
    this.categoryForm = fb.nonNullable.group({ name: ['', [Validators.required, Validators.minLength(2), Validators.maxLength(50)]], parent: [''] });
  }
  ...
  openCreate(): void { this.editing.set(null); this.showUrlFallback.set(false); this.form.reset({ name: '', description: '', category: '', subcategory: '', price: 0, oldPrice: 0, stock: 0, imageUrl: '', featured: false, isNewArrival: false, isManuallyUnavailable: false, gallery: [] }); this.formOpen.set(true); }
  openEdit(product: Product): void { this.editing.set(product); this.showUrlFallback.set(false); this.form.reset({ name: product.name, description: product.description, category: product.category, subcategory: product.subcategory || '', price: product.price, oldPrice: product.oldPrice || 0, stock: product.stock, imageUrl: product.imageUrl, featured: product.featured, isNewArrival: product.isNewArrival, isManuallyUnavailable: product.isManuallyUnavailable || false, gallery: product.gallery || [] }); this.formOpen.set(true); }
  ...
  categoryChanged(): void { this.form.controls.subcategory.setValue(''); }
  subcategories(parent: string): CatalogCategory[] { return this.categories().filter((category) => category.parent === parent).sort((a, b) => a.name.localeCompare(b.name)); }
  openCategoryManager(mode: CategoryMode = 'root', parent = ''): void { this.categoryMode.set(mode); this.categoryManagerOpen.set(true); this.categoryForm.reset({ name: '', parent: mode === 'child' ? (parent || this.rootCategories()[0]?.name || '') : '' }); }
  closeCategoryManager(): void { if (!this.savingCategory()) this.categoryManagerOpen.set(false); }
  chooseCategoryMode(mode: CategoryMode): void { this.categoryMode.set(mode); this.categoryForm.controls.parent.setValue(mode === 'child' ? (this.categoryForm.controls.parent.value || this.form.controls.category.value || this.rootCategories()[0]?.name || '') : ''); }
  prepareSubcategory(parent: string): void { this.categoryMode.set('child'); this.categoryForm.reset({ name: '', parent }); }
  createCategory(): void {
    if (this.categoryForm.invalid || this.savingCategory()) return;
    this.savingCategory.set(true);
    const parent = this.categoryMode() === 'child' ? this.categoryForm.controls.parent.value : '';
    if (this.categoryMode() === 'child' && !parent) { this.savingCategory.set(false); this.toast.error('Choose a main category first'); return; }
    this.admin.createCategory(this.categoryForm.controls.name.value, parent).subscribe({
      next: (category) => { this.categories.update((items) => [...items, category]); if (category.parent) { this.form.controls.category.setValue(category.parent); this.form.controls.subcategory.setValue(category.name); } else { this.form.controls.category.setValue(category.name); this.form.controls.subcategory.setValue(''); } this.categoryForm.reset({ name: '', parent: category.parent || '' }); this.savingCategory.set(false); this.toast.success(category.parent ? 'Subcategory created' : 'Category created'); },
  ...
  save(): void {
    ...
    const payload = { ...raw, category: raw.category.trim().replace(/\s+/g, ' ').toLowerCase(), subcategory: raw.subcategory.trim().replace(/\s+/g, ' ').toLowerCase() || undefined, oldPrice: raw.oldPrice || null };
