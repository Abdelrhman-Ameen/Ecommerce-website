...
@Component({
  ...
  template: `
    ...
    @if (categoryManagerOpen()) { <div class="category-manager-backdrop" (click)="closeCategoryManager()"></div><section class="category-manager-panel"><div class="drawer-head"><div><div class="eyebrow">{{ 'Catalog structure' | translate }}</div><h2>{{ 'Manage categories' | translate }}</h2></div><button type="button" (click)="closeCategoryManager()"><i class="bi bi-x-lg"></i></button></div><form class="category-create-form" [formGroup]="categoryForm" (ngSubmit)="createCategory()"><label for="newCategory">{{ 'New category or subcategory' | translate }}</label><select class="form-select mb-2" formControlName="parent" aria-label="Parent category"><option value="">{{ 'Create as a main category' | translate }}</option>@for (root of rootCategories(); track root.name) { <option [value]="root.name">{{ 'Inside' | translate }} {{ root.name | titlecase }}</option> }</select><div><input id="newCategory" class="form-control" formControlName="name" [placeholder]="'Example: Dresses' | translate"><button class="btn btn-primary-luxe" type="submit" [disabled]="categoryForm.invalid || savingCategory()">{{ (savingCategory() ? 'Adding...' : 'Add category') | translate }}</button></div></form><div class="category-list">@for (category of rootCategories(); track category.name) { <article class="category-root"><div><strong>{{ category.name | titlecase }}</strong><small>{{ category.productCount }} {{ 'products' | translate }} · {{ category.subcategoryCount || 0 }} {{ 'subcategories' | translate }}</small></div>@if (category._id && category.productCount === 0 && !category.subcategoryCount) { <button type="button" (click)="deleteCategory(category)" [attr.aria-label]="'Delete' | translate"><i class="bi bi-trash"></i></button> } @else { <i class="bi bi-lock"></i> }</article>@for (child of subcategories(category.name); track child.name) { <article class="category-child"><div><strong>{{ child.name | titlecase }}</strong><small><i class="bi bi-arrow-return-right me-1"></i>{{ category.name | titlecase }} · {{ child.productCount }} {{ 'products' | translate }}</small></div>@if (child._id && child.productCount === 0) { <button type="button" (click)="deleteCategory(child)" [attr.aria-label]="'Delete' | translate"><i class="bi bi-trash"></i></button> } @else { <i class="bi bi-lock"></i> }</article> } } @empty { <div class="empty-state compact">{{ 'Create your first category.' | translate }}</div> }</div></section> }
  `,
})
export class AdminProductsComponent implements OnInit {
  ...
  openCategoryManager(): void { this.categoryManagerOpen.set(true); this.categoryForm.reset({ name: '', parent: '' }); }
