...
@Component({
  ...
  template: `
    ...
    @if (formOpen()) { <div class="drawer-backdrop" (click)="closeForm()"></div><aside class="admin-drawer"><div class="drawer-head"><div><div class="eyebrow">{{ 'Catalog editor' | translate }}</div><h2>{{ (editing() ? 'Edit product' : 'New product') | translate }}</h2></div><button type="button" (click)="closeForm()"><i class="bi bi-x-lg"></i></button></div><form [formGroup]="form" (ngSubmit)="save()" novalidate>
      ...
      <div class="form-group-luxe"><label for="productPrice">{{ 'Selling price' | translate }} <span class="required-mark">*</span></label><input id="productPrice" class="form-control" type="number" min="0" step="0.01" formControlName="price" [class.is-invalid]="invalid('price')"></div>
...
export class AdminProductsComponent implements OnInit {
  ...
  openCreate(): void { this.editing.set(null); this.form.reset({ name: '', description: '', category: '', collection: '', price: 0, oldPrice: 0, stock: 0, imageUrl: '/assets/catalog/catalog-01.jpg', featured: false, isNewArrival: false, isManuallyUnavailable: false, gallery: [] }); this.formOpen.set(true); }
  openEdit(product: Product): void { this.editing.set(product); this.form.reset({ name: product.name, description: product.description, category: product.category, collection: product.collection || '', price: product.price, oldPrice: product.oldPrice || 0, stock: product.stock, imageUrl: product.imageUrl, featured: product.featured, isNewArrival: product.isNewArrival, isManuallyUnavailable: product.isManuallyUnavailable || false, gallery: product.gallery || [] }); this.formOpen.set(true); }
