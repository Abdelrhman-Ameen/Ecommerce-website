...
@Component({
  ...
  template: `
    <section class="auth-page"><div class="auth-controls"><button class="theme-switch" type="button" (click)="theme.toggle()" [attr.aria-label]="(theme.theme() === 'light' ? 'Enable dark mode' : 'Enable light mode') | translate"><i class="bi" [class.bi-moon-stars]="theme.theme() === 'light'" [class.bi-sun]="theme.theme() === 'dark'"></i></button><button class="language-switch" type="button" (click)="language.toggle()">{{ language.language() === 'en' ? 'عربي' : 'EN' }}</button></div><div class="auth-card"><div class="auth-brand-icon"><i class="bi bi-stars"></i></div><a class="brand-mark justify-content-center mb-1" routerLink="/">Ma3rad El Gamila</a><h1>{{ 'Sign in' | translate }}</h1><p>{{ 'Welcome back. Enter your details to continue.' | translate }}</p>
      ...
      </form><div class="auth-divider"><span>{{ 'or' | translate }}</span></div><p class="mb-0">{{ 'New to Ma3rad El Gamila?' | translate }} <a routerLink="/register">{{ 'Create account' | translate }}</a></p>
