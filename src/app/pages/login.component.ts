...
@Component({
  ...
  template: `
    <section class="auth-page"><button class="auth-language" type="button" (click)="language.toggle()">{{ language.language() === 'en' ? 'عربي' : 'EN' }}</button><div class="auth-card"><div class="auth-brand-icon"><i class="bi bi-stars"></i></div><a class="brand-mark justify-content-center mb-1" routerLink="/">LuxeStudio</a><h1>{{ 'Sign in' | translate }}</h1><p>{{ 'Welcome back. Enter your details to continue.' | translate }}</p>
      ...
      </form><div class="auth-divider"><span>{{ 'or' | translate }}</span></div><p class="mb-0">{{ 'New to LuxeStudio?' | translate }} <a routerLink="/register">{{ 'Create account' | translate }}</a></p>
...
export class LoginComponent {
  ...
  constructor(fb: FormBuilder, private auth: AuthService, private router: Router, private route: ActivatedRoute, public language: LanguageService) {
