    ...
    @else if (product(); as item) {
      <section class="product-detail-section"><div class="container-xxl px-4">
        ...
        <div class="row g-5 align-items-start">
          ...
          <div class="col-lg-5"><div class="product-detail-copy"><div class="eyebrow">{{ (item.subcategory || item.category) | titlecase }}</div><h1>{{ item.name }}</h1><div class="d-flex align-items-center gap-3 mb-4"><strong class="detail-price">{{ item.price | currency }}</strong>@if (item.oldPrice) { <span class="old-price">{{ item.oldPrice | currency }}</span> }</div><p class="detail-description">{{ item.description }}</p>
            <div class="availability-card"><div><span class="stock-dot" [class.out]="!available(item)"></span><strong>{{ available(item) ? (item.stock + ' ' + ('in stock' | translate)) : ('Out of stock' | translate) }}</strong></div><small>{{ (available(item) ? 'Ships within 1–2 business days' : 'Join the restock list') | translate }}</small></div>
            @if (cart.quantityFor(item._id); as cartQuantity) { <div class="cart-quantity-notice" aria-live="polite"><i class="bi bi-bag-check"></i><span><strong>{{ cartQuantity }}</strong> {{ 'of this product in your cart' | translate }}</span><a routerLink="/checkout">{{ 'View cart' | translate }}</a></div> }
            <div class="detail-purchase-row"><div class="quantity-control"><button type="button" (click)="decrease()" [disabled]="quantity() <= 1" [attr.aria-label]="'Decrease quantity' | translate"><i class="bi bi-dash"></i></button><span>{{ quantity() }}</span><button type="button" (click)="increase(availableToAdd(item))" [disabled]="quantity() >= availableToAdd(item)" [attr.aria-label]="'Increase quantity' | translate"><i class="bi bi-plus"></i></button></div><button class="btn btn-primary-luxe flex-grow-1" type="button" [disabled]="!available(item) || availableToAdd(item) < 1 || adding()" (click)="add(item)"><i class="bi bi-bag me-2"></i>{{ (adding() ? 'Adding...' : 'Add to cart') | translate }}</button><button class="btn btn-outline-ink btn-icon" type="button" (click)="favorite(item)" [attr.aria-label]="'Favorite' | translate"><i class="bi" [class.bi-heart-fill]="isFavorite(item)" [class.bi-heart]="!isFavorite(item)"></i></button></div>
            <div class="detail-benefits"><span><i class="bi bi-truck"></i>{{ 'Delivery available' | translate }}</span><span><i class="bi bi-patch-check"></i>{{ 'Authentic' | translate }}</span><span><i class="bi bi-arrow-counterclockwise"></i>{{ '30-day returns' | translate }}</span></div>
...
export class ProductDetailComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private products: ProductService, public cart: CartService, private auth: AuthService, private router: Router, private toast: ToastService) {}
  ngOnInit(): void { this.route.paramMap.subscribe((params) => { const id = params.get('id'); if (id) this.load(id); }); }
  load(id: string): void { this.loading.set(true); this.related.set([]); this.quantity.set(1); this.products.get(id).subscribe({ next: (response) => { const product = response.data.product; this.product.set(product); this.selectedImage.set(product.imageUrl); this.loading.set(false); this.products.recommendations(product._id, 4).subscribe({ next: (related) => this.related.set(related.data.products), error: () => this.related.set([]) }); }, error: () => this.router.navigate(['/404'], { replaceUrl: true }) }); }
