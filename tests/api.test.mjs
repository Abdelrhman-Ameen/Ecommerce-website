...
describe('LuxeStudio API production flow', () => {
  ...
  it('enforces admin authorization and supports product CRUD', async () => {
    ...
    const created = await admin.post('/api/v1/products').send({
      ...
      category: 'testing', collection: 'automated checks', price: 79, stock: 3,
      imageUrl: '/assets/catalog/catalog-01.jpg', gallery: [], rating: 4.8, reviewsCount: 0, featured: false, isNewArrival: false,
    }).expect(201);
  ...
  it('supports favorites, cart CRUD, checkout, and order tracking', async () => {
    ...
    const order = await customer.post('/api/v1/orders').send({ shippingAddress: { fullName: 'Integration Customer', email, phone: '+201000000099', street: '10 Integration Street', city: 'Cairo' } }).expect(201);
